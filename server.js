...
// 🔁 Live Bot Call Handling from AGI (VICIdial)
app.post("/start-bot", async (req, res) => {
  try {
    const audioBuffer = Buffer.from(req.body.audio, "base64"); // audio should be sent as base64
    ...
    recognizeLiveAudio(audioBuffer, lastBotMessage, async (err, result) => {
      ...
      res.json(result); // { userText, intent, action, message }
    });
  } catch (err) {
